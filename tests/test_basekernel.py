...
from kernelbridge.basekernel import Sum
from kernelbridge.basekernel import Product
...
def test_kernel_add_constant(kernel):
    ...
    for kadd in [kernel + 1, 1 + kernel]:
        ...
        assert(len(str(kadd).split('+')) == 2)
        assert(str(kernel) in str(kadd))
        # assert(len(repr(kadd).split('+')) == 2) new repr
        assert(repr(kernel) in repr(kadd))
        ''' hyperparameter retrieval '''
        assert(kernel.theta in kadd.theta)
        print(kadd.theta)
...
def test_kernel_mul_constant(kernel):
    ...
    for kmul in [kernel * 2, 2 * kernel]:
        ...
        assert(len(str(kmul).split('*')) == 2)
        assert(str(kernel) in str(kmul))
        # assert(len(repr(kmul).split('*')) == 2) new repr
        assert(repr(kernel) in repr(kmul))
        ''' hyperparameter retrieval '''
        assert(kernel.theta in kmul.theta)
        kmul.theta = kmul.theta
...
def test_rconv_kernel():
    k = RConvolution(r"A[AGCT]*T")
    # assert(k("AGGGCAACGTACGATCAACT", "ATAGCAGGGCAACGTACGACTAACTCAG") == 0)
    # assert(k("ATAGCAGGGCAACGTACGACTAACTCAG", "ATAGCAGGGCAACGTACGACTAACTCAG") == 4)

    arr1 = ["AGGGCAACGTACGATCAACT", "ATAGCAGGGCAACGTACGACTAACTCAG"]
    arr2 = ["ATAGCAGGGCAACGTACGACTAACTCAG", "ATAGCAGGGCAACGTACGACTAACTCAG"]

    print(k(arr1, arr1))
    print(np.linalg.eigvalsh(k(arr1, arr1)))
>>>>>>> Stashed changes
